...
- **Integration tests** for the target side (apply, ownership records, lock, modrdn, size-limit detection) run against a real OpenLDAP container behind the `integration` build tag. Add the CI job together with the first code, not before. `ci.yml` triggers on Go, module, GoReleaser, and workflow (`ci.yml`, `release.yml`) changes.
